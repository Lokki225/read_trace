# Risk Assessment Template

## Overview

**Feature**: [Feature name]  
**Feature ID**: [ID]  
**Story**: [Story reference]  
**Last Updated**: [YYYY-MM-DD]  
**Risk Assessment Date**: [YYYY-MM-DD]  

This document identifies, analyzes, and provides mitigation strategies for risks associated with this feature.

## Risk Assessment Framework

### Risk Scoring

**Risk Score = Probability × Impact**

Where:
- **Probability**: 1 (Low) to 5 (High)
- **Impact**: 1 (Low) to 5 (High)
- **Risk Score**: 1-25 (categorized as Low, Medium, High, Critical)

### Risk Categories

| Score | Category | Color | Action |
|-------|----------|-------|--------|
| 1-5 | Low | Green | Monitor |
| 6-12 | Medium | Yellow | Plan mitigation |
| 13-20 | High | Orange | Active mitigation required |
| 21-25 | Critical | Red | Escalate immediately |

## Technical Risks

### Risk 1: [Technical Risk Title]

**Risk ID**: TR-001  
**Category**: Technical  
**Severity**: [CRITICAL | HIGH | MEDIUM | LOW]  

**Description**:
[Detailed description of the technical risk]

**Probability**: [1-5] - [Low | Medium | High]  
**Impact**: [1-5] - [Low | Medium | High]  
**Risk Score**: [Probability × Impact] - [Category]  

**Affected Components**:
- [Component 1]
- [Component 2]
- [Component 3]

**Trigger Conditions**:
- [Condition 1]
- [Condition 2]

**Consequences**:
- [Consequence 1]
- [Consequence 2]
- [Consequence 3]

**Mitigation Strategy**:
1. **Prevention**: [How to prevent this risk]
2. **Detection**: [How to detect if this risk occurs]
3. **Response**: [How to respond if it happens]

**Mitigation Owner**: [Name/Role]  
**Timeline**: [When mitigation should be in place]  
**Status**: [Not Started | In Progress | Completed]  

**Contingency Plan**:
[What to do if mitigation fails]

**Related Risks**: [Links to related risks]  

---

### Risk 2: [Technical Risk Title]

**Risk ID**: TR-002  
**Category**: Technical  
**Severity**: [CRITICAL | HIGH | MEDIUM | LOW]  

**Description**:
[Detailed description of the technical risk]

**Probability**: [1-5] - [Low | Medium | High]  
**Impact**: [1-5] - [Low | Medium | High]  
**Risk Score**: [Probability × Impact] - [Category]  

**Affected Components**:
- [Component 1]
- [Component 2]

**Trigger Conditions**:
- [Condition 1]
- [Condition 2]

**Consequences**:
- [Consequence 1]
- [Consequence 2]

**Mitigation Strategy**:
1. **Prevention**: [How to prevent this risk]
2. **Detection**: [How to detect if this risk occurs]
3. **Response**: [How to respond if it happens]

**Mitigation Owner**: [Name/Role]  
**Timeline**: [When mitigation should be in place]  
**Status**: [Not Started | In Progress | Completed]  

**Contingency Plan**:
[What to do if mitigation fails]

## Performance Risks

### Risk 3: [Performance Risk Title]

**Risk ID**: PR-001  
**Category**: Performance  
**Severity**: [CRITICAL | HIGH | MEDIUM | LOW]  

**Description**:
[Detailed description of the performance risk]

**Probability**: [1-5] - [Low | Medium | High]  
**Impact**: [1-5] - [Low | Medium | High]  
**Risk Score**: [Probability × Impact] - [Category]  

**Performance Metrics at Risk**:
- [Metric 1]: Current [value], Target [value]
- [Metric 2]: Current [value], Target [value]

**Trigger Conditions**:
- [Condition 1]
- [Condition 2]

**Consequences**:
- [Consequence 1]
- [Consequence 2]

**Mitigation Strategy**:
1. **Prevention**: [How to prevent this risk]
2. **Detection**: [How to detect if this risk occurs]
3. **Response**: [How to respond if it happens]

**Mitigation Owner**: [Name/Role]  
**Timeline**: [When mitigation should be in place]  
**Status**: [Not Started | In Progress | Completed]  

**Contingency Plan**:
[What to do if mitigation fails]

## Security Risks

### Risk 4: [Security Risk Title]

**Risk ID**: SR-001  
**Category**: Security  
**Severity**: [CRITICAL | HIGH | MEDIUM | LOW]  

**Description**:
[Detailed description of the security risk]

**Probability**: [1-5] - [Low | Medium | High]  
**Impact**: [1-5] - [Low | Medium | High]  
**Risk Score**: [Probability × Impact] - [Category]  

**Security Concerns**:
- [Concern 1]
- [Concern 2]
- [Concern 3]

**Affected Assets**:
- [Asset 1]
- [Asset 2]

**Trigger Conditions**:
- [Condition 1]
- [Condition 2]

**Consequences**:
- [Consequence 1]
- [Consequence 2]

**Mitigation Strategy**:
1. **Prevention**: [How to prevent this risk]
2. **Detection**: [How to detect if this risk occurs]
3. **Response**: [How to respond if it happens]

**Security Controls**:
- [Control 1]
- [Control 2]
- [Control 3]

**Mitigation Owner**: [Name/Role]  
**Timeline**: [When mitigation should be in place]  
**Status**: [Not Started | In Progress | Completed]  

**Compliance Requirements**:
- [Requirement 1]
- [Requirement 2]

**Contingency Plan**:
[What to do if mitigation fails]

## Integration Risks

### Risk 5: [Integration Risk Title]

**Risk ID**: IR-001  
**Category**: Integration  
**Severity**: [CRITICAL | HIGH | MEDIUM | LOW]  

**Description**:
[Detailed description of the integration risk]

**Probability**: [1-5] - [Low | Medium | High]  
**Impact**: [1-5] - [Low | Medium | High]  
**Risk Score**: [Probability × Impact] - [Category]  

**Systems/Services Involved**:
- [System 1]
- [System 2]
- [System 3]

**Integration Points**:
- [Integration point 1]
- [Integration point 2]

**Trigger Conditions**:
- [Condition 1]
- [Condition 2]

**Consequences**:
- [Consequence 1]
- [Consequence 2]

**Mitigation Strategy**:
1. **Prevention**: [How to prevent this risk]
2. **Detection**: [How to detect if this risk occurs]
3. **Response**: [How to respond if it happens]

**Mitigation Owner**: [Name/Role]  
**Timeline**: [When mitigation should be in place]  
**Status**: [Not Started | In Progress | Completed]  

**Contingency Plan**:
[What to do if mitigation fails]

## Business Risks

### Risk 6: [Business Risk Title]

**Risk ID**: BR-001  
**Category**: Business  
**Severity**: [CRITICAL | HIGH | MEDIUM | LOW]  

**Description**:
[Detailed description of the business risk]

**Probability**: [1-5] - [Low | Medium | High]  
**Impact**: [1-5] - [Low | Medium | High]  
**Risk Score**: [Probability × Impact] - [Category]  

**Business Impact**:
- [Impact 1]
- [Impact 2]
- [Impact 3]

**Affected Stakeholders**:
- [Stakeholder 1]
- [Stakeholder 2]

**Trigger Conditions**:
- [Condition 1]
- [Condition 2]

**Consequences**:
- [Consequence 1]
- [Consequence 2]

**Mitigation Strategy**:
1. **Prevention**: [How to prevent this risk]
2. **Detection**: [How to detect if this risk occurs]
3. **Response**: [How to respond if it happens]

**Mitigation Owner**: [Name/Role]  
**Timeline**: [When mitigation should be in place]  
**Status**: [Not Started | In Progress | Completed]  

**Contingency Plan**:
[What to do if mitigation fails]

## Risk Summary Matrix

| Risk ID | Title | Category | Probability | Impact | Score | Status |
|---------|-------|----------|-------------|--------|-------|--------|
| TR-001 | [Title] | Technical | [1-5] | [1-5] | [Score] | [Status] |
| TR-002 | [Title] | Technical | [1-5] | [1-5] | [Score] | [Status] |
| PR-001 | [Title] | Performance | [1-5] | [1-5] | [Score] | [Status] |
| SR-001 | [Title] | Security | [1-5] | [1-5] | [Score] | [Status] |
| IR-001 | [Title] | Integration | [1-5] | [1-5] | [Score] | [Status] |
| BR-001 | [Title] | Business | [1-5] | [1-5] | [Score] | [Status] |

## Risk Monitoring Plan

### Monitoring Schedule

- **Weekly Review**: [Which risks to review weekly]
- **Bi-weekly Review**: [Which risks to review bi-weekly]
- **Monthly Review**: [Which risks to review monthly]

### Key Metrics to Monitor

- [Metric 1]: Target [value], Current [value]
- [Metric 2]: Target [value], Current [value]
- [Metric 3]: Target [value], Current [value]

### Escalation Criteria

Risk should be escalated if:
- Risk score increases by 5+ points
- New trigger conditions are detected
- Mitigation strategy is ineffective
- Timeline for mitigation is at risk

### Escalation Path

1. **Level 1**: [Role/Person]
2. **Level 2**: [Role/Person]
3. **Level 3**: [Role/Person]

## Risk Review History

| Date | Reviewer | Changes | Status |
|------|----------|---------|--------|
| [Date] | [Name] | [Changes made] | [Status] |
| [Date] | [Name] | [Changes made] | [Status] |

## Lessons Learned

### From Previous Similar Features

- [Lesson 1]
- [Lesson 2]
- [Lesson 3]

### Applied to This Feature

- [How lesson 1 is applied]
- [How lesson 2 is applied]
- [How lesson 3 is applied]

## Risk Acceptance

### Accepted Risks

Some risks may be accepted if:
- Mitigation cost exceeds benefit
- Risk is outside team's control
- Risk is acceptable within business context

**Accepted Risk 1**: [Risk description]  
**Reason**: [Why this risk is accepted]  
**Owner**: [Who accepts this risk]  
**Date**: [Date accepted]  

## Sign-off

| Role | Name | Date | Signature |
|------|------|------|-----------|
| Product Owner | | | |
| Tech Lead | | | |
| Risk Owner | | | |

## Appendix

### A. Risk Assessment Methodology

[Describe the methodology used for risk assessment]

### B. Historical Risk Data

[Reference to historical risk data from similar features]

### C. External Risk Factors

[Industry trends, market conditions, regulatory changes that could impact risks]

### D. References

- [Risk management standard or framework]
- [Related documentation]
- [Industry best practices]

---

**Document Status**: [DRAFT | REVIEW | APPROVED | ARCHIVED]  
**Last Reviewed**: [YYYY-MM-DD]  
**Next Review**: [YYYY-MM-DD]  
**Review Frequency**: [Weekly | Bi-weekly | Monthly]
